  ...
  return (
    <div className="overflow-hidden">
      ...
      <div className="hide-scrollbar flex gap-3 overflow-x-auto pb-2 md:gap-4">
        {items.map(item => (
          <a key={`${item.slug}-${item.episodeKey}`} href={`/movie-detail/?slug=${encodeURIComponent(item.slug)}&episode=${encodeURIComponent(item.episodeKey || '')}`} className="group flex w-[205px] shrink-0 gap-2 rounded-lg border border-white/8 bg-[#141414] p-2 hover:border-[#ED2C25]/50 md:w-[220px] md:gap-3">
            <div className="relative h-20 w-14 shrink-0 overflow-hidden rounded-md bg-black md:h-24 md:w-16">
            ...
            <div className="min-w-0 flex-1 py-1">
              ...
              <div className="mt-2 inline-flex items-center gap-1 rounded bg-white/8 px-2 py-1 text-[11px] font-semibold text-white/60 md:mt-3"><Clock3 size={12} /> {formatTime(item.currentTime)}</div>
            </div>
          </a>
        ))}
      </div>
