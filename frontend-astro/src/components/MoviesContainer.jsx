...
export default function MoviesContainer() {
  ...
  const fetchMovies = async (endpoint, page = 1, customTitle) => {
    setLoading(true);
    try {
      let url = endpoint.startsWith('http') ? endpoint : `${NGUONC_API}${endpoint}`;
  ...
  useEffect(() => {
    if (initialSearch) {
      setActiveTab(`/films/search?keyword=${encodeURIComponent(initialSearch)}`);
  ...
  const handleSearch = () => {
    ...
    setFilterGenre('');
    setFilterCountry('');
    setActiveTab(`/films/search?keyword=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        <button onClick={() => { setActiveTab('/films/phim-bo'); setTitle('Phim Bộ'); }} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === '/films/phim-bo' ? 'bg-[#ED2C25] text-white' : 'bg-[#1A1A1A] hover:bg-white/10 text-white/70'}`}>Phim Bộ</button>
        <button onClick={() => { setActiveTab('/films/phim-le'); setTitle('Phim Lẻ'); }} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === '/films/phim-le' ? 'bg-[#ED2C25] text-white' : 'bg-[#1A1A1A] hover:bg-white/10 text-white/70'}`}>Phim Lẻ</button>
        ...
        <select 
          ...
          onChange={e => {
            ...
            if(e.target.value) {
              setActiveTab(`/films/the-loai/${e.target.value}`);
              setTitle('Lọc Thể Loại');
            } else {
