...
class FFmpegWrapper {
  ...
  checkFFmpegExists() {
    if (config.ffmpegBin.includes('/') || config.ffmpegBin.includes('\\')) {
      return fs.existsSync(config.ffmpegBin);
    }
    const result = spawnSync('sh', ['-lc', `command -v ${config.ffmpegBin}`], { stdio: 'ignore' });
  ...
  async getStreamInfo(channelId, channelObj) {
    ...
    return new Promise((resolve) => {
      ...
      const ffprobeExe = config.ffprobeBin || config.ffmpegBin.replace(/ffmpeg(\.exe)?$/, 'ffprobe$1');
      if ((ffprobeExe.includes('/') || ffprobeExe.includes('\\')) && !fs.existsSync(ffprobeExe)) {
