...
echo "Detected: ${DISTRO} ${DISTRO_VER}"
...
NODE_MAJOR=22
INSTALL_NODESOURCE=false
if ! command -v node &>/dev/null; then
  INSTALL_NODESOURCE=true
elif [[ "$(node --version | cut -d. -f1 | tr -d v)" -lt ${NODE_MAJOR} ]]; then
  INSTALL_NODESOURCE=true
fi
...
if [[ "${DISTRO}" == "ubuntu" ]]; then
  ...
  if ${INSTALL_NODESOURCE}; then
    curl -fsSL https://deb.nodesource.com/setup_${NODE_MAJOR}.x | bash -
  fi
  DEBIAN_FRONTEND=noninteractive apt-get install -y \
    nginx ffmpeg nodejs logrotate libnginx-mod-rtmp
  ...
  DEBIAN_FRONTEND=noninteractive apt-get install -y \
    nginx ffmpeg nodejs logrotate
  # libnginx-mod-rtmp available in Debian 12+
  DEBIAN_FRONTEND=noninteractive apt-get install -y libnginx-mod-rtmp || \
...
if [[ -f "${BUILD_DIR}/config/nginx/nhanchilltv-rtmp.conf.template" ]]; then
  sed -e "s#__APP_DIR__#${APP_DIR}#g" \
    "${BUILD_DIR}/config/nginx/nhanchilltv-rtmp.conf.template" > /etc/nginx/nhanchilltv-rtmp.conf
  if ! grep -q "nhanchilltv-rtmp.conf" /etc/nginx/nginx.conf 2>/dev/null; then
    sed -i '/include \/etc\/nginx\/modules-enabled\/\*.conf;/a include /etc/nginx/nhanchilltv-rtmp.conf;' /etc/nginx/nginx.conf 2>/dev/null || true
  fi
fi
...
echo " TV:   http://YOUR_SERVER_IP/tv/"
